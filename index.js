...
class DadJokePlugin extends Plugin {
		...
		powercord.api.commands.registerCommand({
			...
			executor: async () => {
				try {
                    ...
                    if (data.status !== 200) throw new Error(`Invalid response code from API: ${data.status}, expected 200 (OK).`);
	...
	async getData() {
        ...
        }).set('Accept', 'application/json').set('User-Agent', 'Powercord Dad Joke Plugin https://github.com/FC5570/powercord-random-dadjoke')
		return data.body
	}
